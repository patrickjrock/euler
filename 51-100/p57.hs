...
expand (Sum a b) = Sum a (expand b)
expand half = Fraction (Whole 1) (Sum (Whole 2) half)
expand (Fraction a b) = Fraction a (expand b)

base = Sum (Whole 1) half

expansions = iterate expand base

candidate (Fraction (Whole a) (Whole b)) = (length da) > (length db)
  where da = digits a
        db = digits b
